...
if [[ -f "$STATE_FILE" ]]; then
  ...
  if command -v timeout &>/dev/null; then
    GIT_PORCELAIN=$(timeout 5 git status --porcelain 2>/dev/null || echo "__GIT_UNAVAILABLE__")
  elif command -v gtimeout &>/dev/null; then
    GIT_PORCELAIN=$(gtimeout 5 git status --porcelain 2>/dev/null || echo "__GIT_UNAVAILABLE__")
  else
